	...
	class SafeMySQL {
		...
		private function prepareQuery($args) {
			...
			foreach ($array as $i => $part) {
				...
				switch($part) {
					...
					case '?u': $part = $this->createIN($value); break;
		...
		private function escapeIdent($value) {
			if($value) {
				...
				$this->error("Empty value for identifier (?n) placegolder");
			}
		...
		private function createIN($data) {
			if(!is_array($data)) {
				$this->error("value fot IN (?a) placegolder should be array");
				return;
			}
			if(!$data) {
				return 'NULL';
		...
		private function createSET($data) {
			if(!is_array($data)) {
				$this->error("SET (?u) placegolder expects array".gettype($data)." given");
				return;
			}
			if(!$data) {
				$this->error("Empty array for SET (?u) placegolder");
				return;
			} 
			$query = $comma = '';
		...
		private function error($err) {
			$err = __CLASS__.": ".$err;
			if($this->emode == 'error') {
				$err.=". Error initiated in ".$this->caller().", throw";
